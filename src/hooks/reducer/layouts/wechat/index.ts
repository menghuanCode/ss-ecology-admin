...
const wechatAuthorization = (routeString: string) => {
  const state = CryptoJS.AES.encrypt(
    `${new Date().getTime()}${WechatStateFillString}${WechatStateSecretKey}`,
    WechatStateSecretKey,
  );
...
const validateWechatState = (state: string): boolean => {
  const stateString = CryptoJS.AES.decrypt(
...
export const wechatLayoutReducer: ReducerStateWithoutAction<WechatLayoutReducerActionType> = async (
  state,
  action,
) => {
  switch (action.type) {
